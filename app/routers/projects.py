...
def get_all_projects(db: Session = Depends(db_session)):
    all_projects = util.get_all_items(db, models.Projects)
    return all_projects
...
def create_project(
    ...
    project = util.create_new_item(project_dict, db, models.Projects)
    return project
...
def update_project(
    project_id: int,
    project_update: schemas.ProjectUpdateIn,
    user: dict = Depends(get_current_user),
    db: Session = Depends(db_session),
):
    user_id = user.get("id")
    ...
    project = util.update_item(
        project_id, project_update.model_dump(), db, models.Projects, "project"
    )
    return project
...
def delete_project(
    ...
    util.delete_item(project_id, db, models.Projects, "project")
